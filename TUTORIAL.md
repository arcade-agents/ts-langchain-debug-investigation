...
# Build a Slack, Gmail and GoogleCalendar agent with LangChain (TypeScript) and Arcade

In this tutorial you'll build an AI agent using [LangChain](https://js.langchain.com/) with [LangGraph](https://langchain-ai.github.io/langgraphjs/) in TypeScript and [Arcade](https://arcade.dev) that can interact with Slack, Gmail and GoogleCalendar tools — with built-in authorization and human-in-the-loop support.
...
const TOOLS_WITH_APPROVAL = ['PagerdutyApi_UpdateIncidentStatus', 'PagerdutyApi_AddIncidentNote', 'Slack_SendDmToUser', 'Slack_SendMessageToChannel', 'Gmail_SendEmail', 'GoogleCalendar_CreateEvent'];
````
...
- Clone the [repository](https://github.com/arcade-agents/ts-langchain-debug-investigation) and run it
